...
const createMenu = (objeto) => {
  const menu = {
    ...
    pay: () => {
      let total = 0;
      for (let index = 0; index < menu.comsumption.length; index += 1) {
        const item = menu.comsumption[index];
        if (objeto.food[item]) {
          total += objeto.food[item];
        }
        if (objeto.drinks[item]) {
          total += objeto.drinks[item];
        }
      }
      return total * 1.1;
    }
