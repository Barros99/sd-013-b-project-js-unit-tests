...
// - Definir o objeto que a `createMenu()` retorna, mas separadamente- E, depois, definir a função que será atribuída a `order`.
// ```
// const restaurant = {}
//
...
const orderFromMenu = (request, array) => {
  array.push(request);
};

const createMenu = (insertMenu) => {
  const restaurant = {
    fetchMenu: () => insertMenu,
    consumption: [],
    order: (request) => orderFromMenu(request, restaurant.consumption),
    pay: () => {
      let totalSpent = 0;
      for (let index = 0; index < restaurant.consumption.length; index += 1) {
        if (Object.keys(insertMenu.food).includes(restaurant.consumption[index])) {
          totalSpent += insertMenu.food[restaurant.consumption[index]];
        }
        if (Object.keys(insertMenu.drink).includes(restaurant.consumption[index])) {
          totalSpent += insertMenu.drink[restaurant.consumption[index]];
        }
      } 
      const newTotalSpent = parseFloat((totalSpent * 1.1).toFixed(2));
