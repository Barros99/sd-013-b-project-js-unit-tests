...
describe('6 - Implemente os casos de teste para a função `productDetails`', () => {
  it('Verifica se a função `productDetails` tem o comportamento esperado', () => {
    ...
    for (let index = 0; index < productDetails.length; index += 1) {
      assert.strictEqual(typeof (productDetails('firstProduct', 'secondProduct')[index]), 'object');
    }
    ...
    for (let index = 0; index < productDetails.length; index += 1) {
      assert.strictEqual(productDetails('firstProduct', 'secondProduct')[index].details.productId.endsWith('123'), true);
    }
  });
});
