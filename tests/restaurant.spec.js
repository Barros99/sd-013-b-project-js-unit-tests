...
/*
  ...
  IMPORTANTE: FICADAÇA OS TESTES E PASSOS DE ACORDO COM A ORDEM INDA!
...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    assert.ok((Object.keys(createMenu()).length > 1))
    ...
    assert.deepStrictEqual(createMenu(objectTest).fetchMenu(), objectTest)
    ...
    assert.deepStrictEqual(createMenu(objectTest).comsumption, [])
    // ```
    ...
    const output = createMenu(objectTest);
    const checkOrder = (product) => {
      output.order(product);
      if (output.comsumption === product) {
        return true;
      } else {
        return false
      }
