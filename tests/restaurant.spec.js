...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    const objReturned = createMenu({ food: {}, drink: {} }).fetchMenu();
    assert.deepStrictEqual(Object.keys(objReturned), ['food', 'drink'])

    // ```
    // const objetoRetornado = createMenu({ food: {}, drink: {} });
    // objetoRetornado.fetchMenu() // Retorno: { food: {}, drink: {}}

    assert.deepStrictEqual(objReturned, { food: {}, drink: {}});
    ...
    assert.deepStrictEqual(objReturned.consumption, undefined);
    ...
    assert.deepStrictEqual(restaurant.order('sashimi'), ["coxinha", "agua", "sopa", "sashimi"])
    ...
    const createMenuObj = createMenu();
    createMenuObj.order('coxinha')
    createMenuObj.order('agua')
    assert.deepStrictEqual(createMenuObj.order('coxinha'), ['coxinha', 'agua', 'coxinha'])
    ...
    const menuObjReturned = createMenu({
      food: { 
        coxinha: 3.90, 
        sanduiche: 9.90,
      },
